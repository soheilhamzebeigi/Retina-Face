...
class Predict:
    def __init__(self, model_weights_path):
        ...
        resnet56 = Resnet56()
        self.model = resnet56.resnet_v2(input_shape=self.input_shape, depth=self.depth, num_classes=7)
        self.model.compile(loss='categorical_crossentropy',
              optimizer=Adam(lr=resnet56.lr_schedule(100)),
              metrics=['accuracy'])
        model_type = "ResNet_{}".format(self.depth)
        print("network: ", model_type)
        print(self.model.load_weights(model_weights_path))



    def predict_emotion(self, ndarray_image):
        num, width, height, channels = ndarray_image.shape
        ...
        prediction = self.model.predict(ndarray_image)
        return prediction
